...
//AcSiP(+), for power saving
/*!
 * \brief Set power saving duration under seconds scale
 *
 * \param [IN] hrtc RTC handle
 * \param [IN] seconds How many seconds do it need to sleep
 */
void RtcWakeUpTimeSetting( RTC_HandleTypeDef *hrtc, unsigned int seconds );

//AcSiP(+), for power saving
/*!
 * \brief To init RTC for power saving demo version
 *
 * \retval RTC handle
 */
RTC_HandleTypeDef *RtcPowerSavingConfig( void );

//AcSiP(+), for power saving
/*!
 * \brief Before entering power saving, it needs to be called
 */
void RtcRccPowerSavingConfig( void );
