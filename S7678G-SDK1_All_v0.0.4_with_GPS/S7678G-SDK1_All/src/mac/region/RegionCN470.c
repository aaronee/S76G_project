...
//AcSiP(+), for CN470 TX/RX using identical freq
/*!
 * For CN470 TX/RX Frequency
 */
static uint32_t TRX_Frequency = 0;
...
static void EnableChannelsMaskByCount( uint8_t count, uint16_t *channelsMask )
{
    uint8_t index = 0;
    uint8_t position = 0;

    //Set channel mask by increasing index
    for ( int i = 0; i < count; i++ ) {
        channelsMask[index] |= ( 1 << position );

        //Prepare for the next channel mask
        if ( ( ++position ) == 16 ) {
            position = 0;
            index++;
        }
...
void RegionCN470InitDefaults( InitType_t type )
{
    switch( type )
    {
        case INIT_TYPE_INIT:
        {
            // Channels
            // 125 kHz channels
#if ( CN470_USE_NB_CHANNELS != 0 )
            for( uint8_t i = 0; i < CN470_USE_NB_CHANNELS; i++ )
            {
                Channels[i].Frequency = 471500000 + i * 200000;
                Channels[i].DrRange.Value = ( DR_5 << 4 ) | DR_0;
                Channels[i].Band = 0;
            }

            // Initialize the channels default mask
            EnableChannelsMaskByCount( CN470_USE_NB_CHANNELS, ChannelsDefaultMask );
#else
...
//AcSiP(+)
#if ( CN470_USE_NB_CHANNELS != 0 )
                            EnableChannelsMaskByCount( CN470_USE_NB_CHANNELS, ChannelsMask );
#else
...
//AcSiP(m), for CN470 TX/RX using identical freq
#if ( CN470_USE_NB_CHANNELS != 0 )
        frequency = TRX_Frequency;
#else
    ...
    //AcSiP(+)
    UartPrint( "--> RxWindowSetup -- freq=%d datarate=%d sf=%d bandwidth=%d\n",
               frequency, *datarate, phyDr, rxConfig->Bandwidth );
...
bool RegionCN470TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
    ...
    //AcSiP(+), for CN470 TX/RX using identical freq
    TRX_Frequency = Channels[txConfig->Channel].Frequency;
    ...
    *txPower = txPowerLimited;
    ...
    //AcSiP(+)
    UartPrint( "--> Frequency(%d)\n",Channels[txConfig->Channel].Frequency );
    UartPrint( "--> SendFrameOnChannel -- DataRate(%d) TxPower(%d) SF(%d) BW(%d) TxTimeOnAir(%d)\n",
               txConfig->Datarate, phyTxPower, phyDr, 0, *txTimeOnAir );
