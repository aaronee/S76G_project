...
//AcSiP(+), , for set lorawan sync word
/*!
...
static void OnRadioRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
{
    ...
    //AcSiP(m), snr --> snr/4
    UartPrint("--> OnRadioRxDone rssi(%d) snr(%d)\n", rssi, snr/4);
...
static void OnRxWindow1TimerEvent( void )
{
    ...
    //AcSiP(+)
    UartPrint("--> OnRxWindow1TimerEvent\n");
...
static void OnRxWindow2TimerEvent( void )
{
    ...
    //AcSiP(+)
    UartPrint("--> OnRxWindow2TimerEvent\n");
...
static LoRaMacStatus_t ScheduleTx( void )
{
    ...
    {
        //AcSiP(+)
        UartPrint( "--> Tx Send, UpLinkCounter(%d)\n", UpLinkCounter );
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks, LoRaMacRegion_t region )
{
    ...
    //AcSiP(+), Set Ocsillator as TCXO or Crystal type, true = TCXO, false = Crystal
    Radio.SetOscillator( );
